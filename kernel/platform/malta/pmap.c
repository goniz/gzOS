...
static void pmap_change_pte(pmap_t *pmap, vm_addr_t vaddr, vm_prot_t prot) {
    ...
    kprintf("Change protection bits for page %08lx (PTE at %08lx)\n", (vaddr & PTE_MASK),
...
int pmap_probe(pmap_t *pmap, vm_addr_t start, vm_addr_t end, vm_prot_t prot) {
    ...
    while (start < end) {
        ...
        tlblo_t lo0, lo1;
        int tlb_idx = tlb_probe(hi, &lo0, &lo1, 0);
...
struct user_regs *tlb_exception_handler(struct user_regs *regs) {
    int code = (int) ((regs->cause & CR_X_MASK) >> CR_X_SHIFT);
    vm_addr_t vaddr = regs->badvaddr;
